...
class LMMD_loss(nn.Module):
    """Local Maximum Mean Discrepancy for class-conditional distribution alignment."""
    ...
    def convert_to_onehot(self, sca_label, class_num=2):
        return np.eye(class_num)[sca_label]

    def cal_weight(self, s_label, t_label, batch_size=32, class_num=2):
...
class DSAN(nn.Module):
    """Deep Subdomain Adaptation Network using LMMD."""
    ...
    def forward(self, source, target, s_label):
        ...
        s_pred = self.classifier(source_features)

        target_features = self.feature_extractor(target)
        t_pred = self.classifier(target_features)

        loss_lmmd = self.lmmd_loss.get_loss(
            source_features,
            target_features,
            s_label,
            torch.nn.functional.softmax(t_pred, dim=1),
        )
